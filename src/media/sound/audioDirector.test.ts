...
it.each(['runner','hoofed'] as const)('%s routes to one motion loop, stops at rest and cleans on switch', async style => {
  vi.resetModules(); howls.length=0; sessionStorage.clear()
  const {raceAudio}=await import('./audioDirector')
  raceAudio.beginRace('sport','animal',style)
  const loops=howls.filter(h=>h.loop)
  expect(loops).toHaveLength(1)
  const state={speed:70,maxSpeed:100,accelerating:true,braking:false}
  raceAudio.syncEngine({...state,speed:0})
  expect(loops[0].playingFlag).toBe(false)
  raceAudio.syncEngine(state); raceAudio.syncEngine(state)
  expect(loops[0].play).toHaveBeenCalledTimes(1)
  const fastRate=loops[0].rateValue
  raceAudio.syncEngine({...state,speed:20})
  expect(loops[0].rateValue).toBeLessThan(fastRate)
  raceAudio.syncEngine({...state,speed:0})
  expect(loops[0].playingFlag).toBe(false)
  raceAudio.syncEngine(state); raceAudio.setMuted(true)
  expect(loops[0].playingFlag).toBe(false)
  raceAudio.setMuted(false)
  raceAudio.beginRace('electric','car','vehicle')
  expect(loops[0].unload).toHaveBeenCalled()
  raceAudio.syncEngine(state)
  raceAudio.beginRace('light','restart',style)
  expect(howls.filter(h=>h.loop&&h.playingFlag)).toHaveLength(0)
  raceAudio.syncEngine(state); raceAudio.dispose()
  expect(howls.filter(h=>h.loop&&h.playingFlag)).toHaveLength(0)
})
